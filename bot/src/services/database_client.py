...
class DatabaseClient:
    ...
    async def change_access(self, telegram_id: int, group_id: int) -> bool:
        if self.stub is None or database_pb2 is None:
            return True
        resp = await self.stub.ChangeAccess(
            database_pb2.ChangeAccessRequest(telegram_id=telegram_id, group_id=group_id)
        )
        return bool(resp.ok)
    ...
    async def is_owner(self, telegram_id: int) -> bool:
        if self.stub is None or database_pb2 is None:
            return False
        resp = await self.stub.IsOwner(
            database_pb2.IsAccessRequest(telegram_id=telegram_id)
        )
        return bool(resp.ok)
    ...
    async def delete_item(self, telegram_id: int, hash_name: str) -> bool:
        ...
        resp = await self.stub.DeleteItem(
            database_pb2.DeleteItemRequest(telegram_id=telegram_id, hash_name=hash_name)
        )
        return bool(resp.ok)
    ...
    async def user_items(self, telegram_id: int) -> Dict[str, int]:
        if self.stub is None or database_pb2 is None:
            return {}
        resp = await self.stub.UserItems(
            database_pb2.UserItemsRequest(telegram_id=telegram_id)
        )
        return dict(resp.items)

    async def add_log(
        self, telegram_id: int, username: Optional[str], function_name: str
    ) -> None:
        if self.stub is None or database_pb2 is None:
            return
        await self.stub.AddLog(
            database_pb2.AddLogRequest(
